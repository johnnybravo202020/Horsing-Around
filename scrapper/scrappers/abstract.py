...
class BasePageScrapper:
    ...
    def __init__(self, html='', url='', save_data_for_test=False):
...
class BaseRaceDayScrapper(BasePageScrapper):
    ...
    def __init__(self, city, date, html='', url='', save_data_for_test=False, get_past_statistics=False):
        self.get_past_statics = get_past_statistics
        self.race_day = ''
        self.city = city
        self.date = date

        super(BaseRaceDayScrapper, self).__init__(html, url, save_data_for_test)
    ...
    def get(self):
        ...
        for rDiv in self.race_divs:
            ...
            for i, row in enumerate(horse_rows):
                ...
                if self.save_data_for_test:
                    test = self.test_model.from_actual(model, row)
    ...
    @classmethod
    def scrap_by_date(cls, city, date, save_data_for_test=True, get_past_statistics=False):
        """
        ...
        """
        scrapper = cls(city, date, save_data_for_test=save_data_for_test, get_past_statistics=get_past_statistics)
        return scrapper.get()

    @classmethod
    def scrap(cls, city, year, month, day, save_data_for_test=True, get_past_statistics=False):
        """
        ...
        :return: Returns the results of the desired race
        """
        return cls.scrap_by_date(city, datetime.datetime(year, month, day), save_data_for_test, get_past_statistics)
